...
def get_data(symbol, interval="15m"):
    # 1. Indian Market (Alternative Source)
    if symbol in ["NIFTY50", "BANKNIFTY"]:
        ticker = "%5ENSEI" if symbol == "NIFTY50" else "%5ENSEBANK"
        ...
        try:
            ...
            print(f"Error Indian Data: {e}")
            return pd.DataFrame()

    # 2. Crypto Market (Binance Direct - Most Stable)
    else:
...
def dashboard():
    ...
    for sym in SYMBOLS:
        ...
        price_display = f"{c:,.2f}" if c > 0 else "Loading..."
    ...
    return f"""
    <html>
    <head><meta http-equiv="refresh" content="30"><title>Pro Trading Hub</title></head>
    <body style="background:#0e1117; color:white; font-family:sans-serif; padding:20px; margin:0;">
    <h2 style='text-align:center; color:#00d1ff; margin-bottom:30px;'>🚀 PRO TRADING HUB (v2.0 Fixed)</h2>
    <div style="display:flex; flex-wrap:wrap; gap:20px; justify-content:center;">
        <div style="flex:1; min-width:300px; max-width:400px; background:#1a1c24; border-radius:12px; padding:20px; border:1px solid #333;">
        ...
        <div style="flex:2; min-width:500px; background:#1a1c24; border-radius:12px; padding:20px; border:1px solid #333;">
            <h3 style='margin-top:0;'>Live Chart (BTC/USDT)</h3> {get_chart()}
        </div>
    </div>
    <div style="margin-top:20px; background:#1a1c24; border-radius:12px; padding:20px; border:1px solid #333;">
        ...
        <table style='width:100%; border-collapse: collapse; text-align:left;'>
            ...
            {journal_rows if journal_rows else "<tr><td colspan='5' style='text-align:center; padding:30px; color:#555;'>Waiting for first signal...</td></tr>"}
        </table>
    </div>
    </body>
    </html>
